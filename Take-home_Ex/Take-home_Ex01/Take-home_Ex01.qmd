...
From our analysis above, noting that the limitation of nearest neighbour distance method is that it uses only nearest distance;

The L function is an adjusted form of the K function that can be plotted as a straight line under complete spatial randomness (CSR). This makes it easier to detect deviations from randomness, i.e., clustering or dispersion.In practice, K function will be normalised to obtained a benchmark of zero.

Hence, it can be used in helping to intepret how accident densities vary over distance, making it perfect for investigating whether accidents cluster at different scales (e.g., across neighbourhoods or road networks).

For this section, L-function estimates will be computed by using [*Lest()*](https://rdrr.io/cran/spatstat/man/Lest.html) of **spatstat** package. A Monte Carlo simulation test will also be carried out using [*envelope()*](https://rdrr.io/cran/spatstat/man/envelope.html) of **spatstat** package.

To confirm the observed spatial patterns above, a hypothesis test will be conducted. The hypothesis results will allow further observation confirmation if accidents in the Bangkok Metropolitan Region are Randomly Distributed and if they exhibit Spatial Clustering or Randomness.
...
```{r}
L_bk = Lest(bmr_accidents_bk_ppp, correction = "Ripley")
plot(L_bk, . -r ~ r, 
     ylab= "L(d)-r", xlab = "d(m)",
     xlim=c(0,1000))
```
...
```{r}
L_bk.csr <- envelope(bmr_accidents_bk_ppp, Kest, nsim = 99, rank = 1, glocal=TRUE)
```

```{r}
plot(L_bk.csr, . - r ~ r, xlab="d", ylab="K(d)-r")
```
...
```{r}
L_sp = Lest(bmr_accidents_sp_ppp, correction = "Ripley")
plot(L_sp, . -r ~ r, 
     ylab= "L(d)-r", xlab = "d(m)",
     xlim=c(0,1000))
```
...
```{r}
L_sp.csr <- envelope(bmr_accidents_sp_ppp, Kest, nsim = 99, rank = 1, glocal=TRUE)
```

```{r}
plot(L_sp.csr, . - r ~ r, xlab="d", ylab="K(d)-r")
```
...
```{r}
L_pt = Lest(bmr_accidents_pt_ppp, correction = "Ripley")
plot(L_pt, . -r ~ r, 
     ylab= "L(d)-r", xlab = "d(m)",
     xlim=c(0,1000))
```
...
```{r}
L_pt.csr <- envelope(bmr_accidents_pt_ppp, Kest, nsim = 99, rank = 1, glocal=TRUE)
```

```{r}
plot(L_pt.csr, . - r ~ r, xlab="d", ylab="K(d)-r")
```
...
```{r}
L_np = Lest(bmr_accidents_np_ppp, correction = "Ripley")
plot(L_np, . -r ~ r, 
     ylab= "L(d)-r", xlab = "d(m)",
     xlim=c(0,1000))
```
...
```{r}
L_np.csr <- envelope(bmr_accidents_np_ppp, Kest, nsim = 99, rank = 1, glocal=TRUE)
```

```{r}
plot(L_np.csr, . - r ~ r, xlab="d", ylab="K(d)-r")
```
...
```{r}
L_ss = Lest(bmr_accidents_ss_ppp, correction = "Ripley")
plot(L_ss, . -r ~ r, 
     ylab= "L(d)-r", xlab = "d(m)",
     xlim=c(0,1000))
```
...
```{r}
L_ss.csr <- envelope(bmr_accidents_ss_ppp, Kest, nsim = 99, rank = 1, glocal=TRUE)
```

```{r}
plot(L_ss.csr, . - r ~ r, xlab="d", ylab="K(d)-r")
```
...
```{r}
L_nt = Lest(bmr_accidents_nt_ppp, correction = "Ripley")
plot(L_nt, . -r ~ r, 
     ylab= "L(d)-r", xlab = "d(m)",
     xlim=c(0,1000))
```
...
```{r}
L_nt.csr <- envelope(bmr_accidents_nt_ppp, Kest, nsim = 99, rank = 1, glocal=TRUE)
```

```{r}
plot(L_nt.csr, . - r ~ r, xlab="d", ylab="K(d)-r")
```
:::
